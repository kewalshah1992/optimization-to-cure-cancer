...
#minimize dosage: (sum{i in BEAM} sum {j in ROWS, k in COLS} s[j,k] * X[i] * beam_value[i,j,k]) - (sum{i in BEAM} sum {j in ROWS, k in COLS} e[j,k] * X[i] * beam_value[i,j,k]) + (sum{i in BEAM} sum {j in ROWS, k in COLS} a[j,k] * X[i] * beam_value[i,j,k]);

# minimize total slacks
minimize total_slacks: sum {j in ROWS, k in COLS} (10*(P[j,k] + Q[j,k])) + (sum{i in BEAM} sum {j in ROWS, k in COLS} e[j,k] * X[i] * beam_value[i,j,k]) + (sum{i in BEAM} sum {j in ROWS, k in COLS} a[j,k] * X[i] * beam_value[i,j,k]);
